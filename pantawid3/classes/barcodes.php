...
/**
 * Table class for barcodes
 */
class barcodes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'barcodes';
		$this->TableName = 'barcodes';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`barcodes`";
		$this->Dbid = 'DB';
		...
		// inventory_id
		$this->inventory_id = new DbField('barcodes', 'barcodes', 'x_inventory_id', 'inventory_id', '`inventory_id`', '`inventory_id`', 3, 11, -1, FALSE, '`inventory_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// concat_id
		$this->concat_id = new DbField('barcodes', 'barcodes', 'x_concat_id', 'concat_id', 'concat(\'INV-R6\',inventory_id)', 'concat(\'INV-R6\',inventory_id)', 200, 17, -1, FALSE, 'concat(\'INV-R6\',inventory_id)', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->concat_id->IsCustom = TRUE; // Custom field
		$this->concat_id->Sortable = TRUE; // Allow sort
		$this->fields['concat_id'] = &$this->concat_id;

		// item_serial
		$this->item_serial = new DbField('barcodes', 'barcodes', 'x_item_serial', 'item_serial', '`item_serial`', '`item_serial`', 200, 50, -1, FALSE, '`item_serial`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_serial->Sortable = TRUE; // Allow sort
		$this->fields['item_serial'] = &$this->item_serial;

		// Year_Purchased
		$this->Year_Purchased = new DbField('barcodes', 'barcodes', 'x_Year_Purchased', 'Year_Purchased', '`Year_Purchased`', '`Year_Purchased`', 3, 11, -1, FALSE, '`Year_Purchased`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// item_model
		$this->item_model = new DbField('barcodes', 'barcodes', 'x_item_model', 'item_model', '`item_model`', '`item_model`', 200, 50, -1, FALSE, '`item_model`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// office_id
		$this->office_id = new DbField('barcodes', 'barcodes', 'x_office_id', 'office_id', '`office_id`', '`office_id`', 3, 11, -1, FALSE, '`office_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		switch ($CurrentLanguage) {
			case "en":
				$this->office_id->Lookup = new Lookup('office_id', 'tbl_off', FALSE, 'off_id', ["off_desc","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->office_id->Lookup = new Lookup('office_id', 'tbl_off', FALSE, 'off_id', ["off_desc","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// name_accountable
		$this->name_accountable = new DbField('barcodes', 'barcodes', 'x_name_accountable', 'name_accountable', '`name_accountable`', '`name_accountable`', 3, 50, -1, FALSE, '`name_accountable`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`barcodes`";
	}
	...
	{
		return ($this->SqlSelect != "") ? $this->SqlSelect : "SELECT *, concat('INV-R6',inventory_id) AS `concat_id` FROM " . $this->getSqlFrom();
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->concat_id->DbValue = $row['concat_id'];
	...
	{
		global $Language;
		if ($pageName == "barcodesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "barcodesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "barcodesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("barcodesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("barcodesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "barcodesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "barcodesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barcodesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("barcodesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("barcodesdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->concat_id->setDbValue($rs->fields('concat_id'));
	...
	public function renderEditRow()
	{
		...
		// concat_id
		$this->concat_id->EditAttrs["class"] = "form-control";
		$this->concat_id->EditCustomAttributes = "";
		if (!$this->concat_id->Raw)
			$this->concat_id->CurrentValue = HtmlDecode($this->concat_id->CurrentValue);
		$this->concat_id->EditValue = $this->concat_id->CurrentValue;
		$this->concat_id->PlaceHolder = RemoveHtml($this->concat_id->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->concat_id);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->concat_id);
