...
/**
 * Table class for tbl_year
 */
class tbl_year extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'tbl_year';
		$this->TableName = 'tbl_year';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_year`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// year_id
		$this->year_id = new DbField('tbl_year', 'tbl_year', 'x_year_id', 'year_id', '`year_id`', '`year_id`', 3, 11, -1, FALSE, '`year_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->year_id->IsAutoIncrement = TRUE; // Autoincrement field
		...
		// year_dsc
		$this->year_dsc = new DbField('tbl_year', 'tbl_year', 'x_year_dsc', 'year_dsc', '`year_dsc`', '`year_dsc`', 200, 50, -1, FALSE, '`year_dsc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`tbl_year`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->year_id->setDbValue($conn->insert_ID());
			$rs['year_id'] = $this->year_id->DbValue;
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "tbl_yearedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tbl_yearadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("tbl_yearview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_yearview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "tbl_yearadd.php?" . $this->getUrlParm($parm);
		else
			$url = "tbl_yearadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_yearedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_yearadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_yeardelete.php", $this->getUrlParm());
	}
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->year_id->CurrentValue != NULL) {
			$url .= "year_id=" . urlencode($this->year_id->CurrentValue);
		} else {
