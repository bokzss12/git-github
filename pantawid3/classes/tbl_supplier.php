 ...
 */
class tbl_supplier extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// supplierID
		$this->supplierID = new DbField('tbl_supplier', 'tbl_supplier', 'x_supplierID', 'supplierID', '`supplierID`', '`supplierID`', 3, 11, -1, FALSE, '`supplierID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// supplier_dsc
		$this->supplier_dsc = new DbField('tbl_supplier', 'tbl_supplier', 'x_supplier_dsc', 'supplier_dsc', '`supplier_dsc`', '`supplier_dsc`', 200, 100, -1, FALSE, '`supplier_dsc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->supplier_dsc->Sortable = TRUE; // Allow sort
		$this->fields['supplier_dsc'] = &$this->supplier_dsc;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`tbl_supplier`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->supplier_dsc->DbValue = $row['supplier_dsc'];
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("tbl_supplierview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_supplierview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "tbl_supplieradd.php?" . $this->getUrlParm($parm);
		else
	...
	{
		$url = $this->keyUrl("tbl_supplieredit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_supplieradd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_supplierdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->supplier_dsc->setDbValue($rs->fields('supplier_dsc'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->supplier_dsc->EditValue = $this->supplier_dsc->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->supplier_dsc);
				} else {
					$doc->exportCaption($this->supplierID);
					$doc->exportCaption($this->supplier_dsc);
				}
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->supplier_dsc);
					} else {
						$doc->exportField($this->supplierID);
						$doc->exportField($this->supplier_dsc);
					}
	...
	$rsnew["supplier_dsc"]=mb_strtoupper($rsnew["supplier_dsc"]);
		return TRUE;
	}
