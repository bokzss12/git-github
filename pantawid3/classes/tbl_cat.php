...
/**
 * Table class for tbl_cat
 */
class tbl_cat extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'tbl_cat';
		$this->TableName = 'tbl_cat';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tbl_cat`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// cat_id
		$this->cat_id = new DbField('tbl_cat', 'tbl_cat', 'x_cat_id', 'cat_id', '`cat_id`', '`cat_id`', 3, 11, -1, FALSE, '`cat_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// cat_desc
		$this->cat_desc = new DbField('tbl_cat', 'tbl_cat', 'x_cat_desc', 'cat_desc', '`cat_desc`', '`cat_desc`', 200, 50, -1, FALSE, '`cat_desc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// request_id
		$this->request_id = new DbField('tbl_cat', 'tbl_cat', 'x_request_id', 'request_id', '`request_id`', '`request_id`', 3, 11, -1, FALSE, '`request_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->request_id->Nullable = FALSE; // NOT NULL field
		$this->request_id->Required = TRUE; // Required field
		$this->request_id->Sortable = FALSE; // Allow sort
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`tbl_cat`";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('cat_id', $row) ? $row['cat_id'] : NULL;
		else
			$val = $this->cat_id->OldValue !== NULL ? $this->cat_id->OldValue : $this->cat_id->CurrentValue;
		if (!is_numeric($val))
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "tbl_catedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tbl_catadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("tbl_catview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tbl_catview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "tbl_catadd.php?" . $this->getUrlParm($parm);
		else
			$url = "tbl_catadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_catedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tbl_catadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tbl_catdelete.php", $this->getUrlParm());
	}
	...
	$rsnew["cat_desc"]=mb_strtoupper($rsnew["cat_desc"]);
		return TRUE;
	}
